    ...
    <el-card>
      ...
      <div class="action-section">
        <div class="action-row">
          <span class="action-label">版本号</span>
          <el-input v-model="buildVersion" placeholder="如：v1.1" style="width:200px" />
...
const handleBuild = async () => {
  if (!buildVersion.value.trim()) { ElMessage.warning('请输入版本号'); return }
  building.value = true
  statusMessage.value = ''
  try {
    ...
    ElMessage.error('构建升级包失败：' + (e.message || '未知错误'))
  } finally {
...
.action-row { display:flex; align-items:center; gap:12px; margin-bottom:16px; flex-wrap:wrap; }
.action-label { font-size:14px; color:#606266; width:60px; }
.action-tip { font-size:12px; color:#909399; }
