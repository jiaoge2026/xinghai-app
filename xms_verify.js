...
(async () => {
  ...
  console.log('PAGE TEXT:', body.substring(0, 800));
  ...
  for (const i of allInputs) {
    ...
    const cls = await i.getAttribute('class');
    console.log('  input:', type, '|', ph, '|', cls);
  }

  // Try to fill verification code
  try {
    await page.locator('input[placeholder="请输入验证码"]').fill('655398');
    console.log('SUCCESS: Filled verification code 655398');
    ...
    console.log('Current URL after confirm:', page.url());
    const newBody = await page.textContent('body');
    console.log('Page after confirm:', newBody.substring(0, 500));
  } catch(e) {
